            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="overflow-auto space-y-4 p-4">
                            ...
                            <div class="flex items-end justify-start">
                             ...
                             <div class="flex items-end {{ $message->sender_id == Auth::id() ? 'justify-end' : 'justify-start' }}">
                              <div class="{{ $message->receiver_id == Auth::id() ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800' }} max-w-xs lg:max-w-md text-sm p-2 rounded-lg shadow">
                                 <p>{{ $message->content }}</p>
                              </div>
                             </div>
                             @endforeach
                        ...
                        <!-- メッセージ入力エリア -->
                        <div class="mt-4">
                            <form action="{{ route('messages.send', ['roomId' => $roomId]) }}" method="POST"> 
                              @csrf
